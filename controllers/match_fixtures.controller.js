...
const updateOne = async (req, res) => {
    try {
        ...
        if (updatedFixture.rows.length === 0) {
            res.status(404).json({ message: `Not found such an id of a fixture ${id}` })
        }
        return res.status(200).json({
            message: `Successfully updated a fixture`,
            fixture: updatedFixture.rows[0]
        })
    }
    catch (error) {
        console.log(error)
...
const createOne = async (req, res) => {
    try {
        ...
        if (match_date && venue && home_team_id && away_team_id && home_score && away_score && tournament_id && match_status){
            ...
            const query = `Insert into match_fixtures (match_date, venue, home_team_id, away_team_id, home_score, away_score, tournament_id, match_status) values ($1,$2, $3, $4, $5, $6, $7, $8) returning *`
...
const deleteOne = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: `Successfully deleted Match Fixtures!`, fixture: deletedFixtures.rows[0] })
    } catch (error) {
        console.log(error)
...
const filterAll = async (req, res) => {
    try {
        ...
        if (filter) {
            ...
            const query = `Select * from match_fixtures where venue ilike $1 or match_status ilike $1 offset $2 limit $3`
